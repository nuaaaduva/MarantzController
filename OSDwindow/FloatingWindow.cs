...
namespace MrSmarty.CodeProject
{
	...
	public class FloatingWindow : NativeWindow, IDisposable
    {
		...
		{
            //         Win32.User.PostMessage(base.Handle, Win32.User.WM_PAINT, 0, 0);
            //this.UpdateLayeredWindow();
            //Win32.RECT rect = new Win32.RECT();// { Left = this.X, Top = this.Y, Right = this.X + this.Width, Bottom = this.Y + this.Height }; ;

            Win32.User.SetWindowPos(base.Handle, Win32.User.HWND_TOP, this.Location.X, this.Location.Y, this.Size.Width, this.Size.Height, Win32.User.SWP_NOCOPYBITS | Win32.User.SWP_NOACTIVATE);
            //Win32.User.GetClientRect(base.Handle, ref rect);
            //Win32.User.RedrawWindow(base.Handle, ref rect, IntPtr.Zero, Win32.User.RDW_UPDATENOW | Win32.User.RDW_ERASE | Win32.User.RDW_INVALIDATE );
		}
		...
		protected override void WndProc(ref Message m)
		{
			...
			{
                //System.Diagnostics.Debug.Print("RePaint1");
                this.PerformWmPaint_WmPrintClient(ref m, true);
				return;
			}
            else if(m.Msg == 0x318) // WM_PRINTCLIENT
            {
                //System.Diagnostics.Debug.Print("RePaint2");
